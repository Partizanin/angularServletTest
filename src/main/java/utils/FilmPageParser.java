...
public class FilmPageParser {
    ...
    private String filmsUrls = "http://fs.to/video/films/?sort=rating";

    public static void main(String[] args) {

        FilmPageParser filmPageParser = new FilmPageParser();
        ArrayList<Film> films = filmPageParser.getFilms(18, 1);
        for (Film film : films) {
    ...
    public ArrayList<Film> getFilms(int filmsCount, int filmsPage) {
        ArrayList<Film> films = new ArrayList<Film>();

        for (String filmUrl : getFilmsUrlsFromPages(filmsCount, filmsPage)) {

            String[] names = getNames(filmUrl);

            films.add(new Film(names[0], getYear(filmUrl), getCountries(filmUrl), names[1],
                    getRates(filmUrl), filmUrl, getImgURl(filmUrl), getGenres(filmUrl), LocalDate.now()));
        }
    ...
    private ArrayList<String> getFilmsUrlsFromPages(int filmCount, int filmsPage) {
        ArrayList<String> strings = new ArrayList<>();

        int pageCount = getFilmsPageCount(filmCount) + filmsPage;

        for (int i = filmsPage; i < pageCount; i++) {
    ...
    int getFilmsPageCount(int filmsCount) {
